...
public class MainActivity extends AppCompatActivity {
    Button btn_inflate;//레이아웃 인플레이션(자바 <-> xml) 연결이 되고나서 초기화를 시킬수 있다(디자인에 있는 위젯을 연결 findViewById
    
    //인터페이스 ( 자바에서 어떤 객체의 사용방법을 정의해놓은것) <- 설계도
    // 옵저버 패턴(정의, 구현, 실행) 세개로 나눔   (어떤 동작이 일어날때를 대비했다가 동작이 일어났을때 구현)
    ...
    // 3. 자바코드를 이용해서 레이아웃 인플레이션 과정을 만듬. <= flagment라는 기능의 구조를 이해하기위해서는 반드시 알아야함.
    LinearLayout linear;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        LayoutInflater inflater = getLayoutInflater();  // 레이아웃을 붙일때 필요한것
                                                        // 첫번째 방법
         //inflate()메소드 : ()안의사용법  @LayoutRes int resource, ViewGroup root, boolean attachToRoot
        // inflater = (LayoutInflater) getSystemService(Context.LAYOUT_INFLATER_SERVICE);   //두번째 방법
