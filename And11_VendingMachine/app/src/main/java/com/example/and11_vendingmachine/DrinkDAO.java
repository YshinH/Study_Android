...
public class DrinkDAO {
    ...
    // DrinkDTO 두 건을 가진 ArrayList를 return하는 메소드 initDrinks를 만드세요.

    public ArrayList<DrinkDTO> initDrinks(){
        ArrayList<DrinkDTO> list = new ArrayList<>();
        list.add(new DrinkDTO("스파클링", 1000, 10));
        list.add(new DrinkDTO("사이다", 1000, 10));

        return list;
    }

    //ArrayList<ViewDTO> <- 텍스트이름1, 수량1, 버튼1, .. 2, 2, 2
    //ArrayList<DrinkDTO>를 입력받아 ArrayList<ViewDTO>의 내용을 세팅할 수 있는 메소드를 만드세요.
    //두개의 ArrayList는 Size가 같기때문에 index(주소)를 잘 활용하면 데이터 매칭이 편리하다
